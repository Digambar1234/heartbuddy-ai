import { ArrowRight, Brain, CheckCircle2, Heart, LockKeyhole, MessageCircleHeart, ShieldCheck, Sparkles, TrendingUp, UserRound } from "lucide-react";
...
export default function LandingPage() {
  return (
    <main className="premium-bg min-h-screen overflow-hidden">
      <nav className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-4 px-4 py-5 sm:flex-nowrap">
        <Link to="/" className="flex min-w-0 items-center gap-3 text-lg font-black text-purple-950 sm:text-xl">
          <span className="grid h-10 w-10 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-rose-500 to-purple-700 text-white shadow-glow sm:h-11 sm:w-11">
            <Heart className="h-5 w-5" />
          </span>
          <span className="truncate">HeartBuddy AI</span>
        </Link>
        <div className="grid w-full grid-cols-2 gap-2 sm:flex sm:w-auto sm:shrink-0 sm:gap-3">
          <Link to="/login" className="min-w-0"><Button variant="secondary" className="w-full px-4 sm:w-auto">Login</Button></Link>
          <Link to="/register" className="min-w-0"><Button className="w-full px-4 sm:w-auto" icon={<ArrowRight className="h-4 w-4" />}>Start</Button></Link>
        </div>
      </nav>

      <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-12 pt-6 sm:pt-8 lg:grid-cols-[1.02fr_0.98fr] lg:items-center">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
          <Badge>Memory-aware emotional companion</Badge>
          <h1 className="text-balance mt-5 text-4xl font-black leading-[0.98] text-purple-950 sm:mt-6 sm:text-5xl md:text-7xl">
            Meet HeartBuddy AI
          </h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-700 sm:mt-6 sm:text-xl">
          ...
          <div className="mt-7 grid gap-3 sm:mt-8 sm:flex sm:flex-wrap">
            <Link to="/register"><Button className="w-full sm:w-auto" icon={<ArrowRight className="h-4 w-4" />}>Start your companion</Button></Link>
            <Link to="/login"><Button className="w-full sm:w-auto" variant="secondary">I already have an account</Button></Link>
          </div>
          <p className="mt-5 inline-flex items-center gap-2 rounded-full border border-white/70 bg-white/70 px-4 py-2 text-sm font-black text-slate-800 shadow-sm">
            <UserRound className="h-4 w-4 text-rose-500" />
            Developed by Digambar Shukla
          </p>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.12, duration: 0.45 }}>
          <div className="hero-sheen relative rounded-[2rem] p-3 shadow-glow">
            ...
            <div className="absolute bottom-4 left-4 right-4 rounded-2xl border border-white/70 bg-white/84 p-4 shadow-xl backdrop-blur-xl sm:bottom-6 sm:left-6 sm:right-6 sm:p-5">
