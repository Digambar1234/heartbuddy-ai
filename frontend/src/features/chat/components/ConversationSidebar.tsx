  ...
  if (compact) {
    return (
      <div className="flex gap-2 overflow-x-auto pb-1">
        <Button className="min-w-44 shrink-0" icon={<MessageCirclePlus className="h-4 w-4" />} onClick={onNew} type="button">
          New chat
        </Button>
        {conversations.length > 0 ? (
          <>
            {conversations.map((conversation) => (
              <button
                ...
                className={`min-w-40 shrink-0 rounded-2xl px-3 py-2 text-left transition ${
                  activeConversationId === conversation.id ? "bg-purple-700 text-white" : "bg-white/80 text-slate-700 hover:bg-purple-50"
                }`}
              >
                <p className="truncate text-sm font-black">{conversation.title ?? "New conversation"}</p>
                <p className={`text-xs ${activeConversationId === conversation.id ? "text-purple-100" : "text-slate-500"}`}>
          ...
          <p className="min-w-44 shrink-0 rounded-2xl bg-white/65 px-4 py-3 text-sm font-medium text-slate-500">No conversations yet.</p>
        )}
      </div>
    );
  }
